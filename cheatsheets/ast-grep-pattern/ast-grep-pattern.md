...
### 核心概念
- **抽象语法树 (AST)**：源代码语法结构的抽象表示，丢弃了括号、分号、空格等对编译器不重要的细节
- **具体语法树 (CST)**：忠实地记录源代码的所有字符，包括空格、注释、标点符号等
- ast-grep 内部主要依赖 CST，支持无损重写，保留原始代码格式

### AST 与 CST 的辩证关系
- AST 适合语义分析，CST 适合代码重构和格式化
- ast-grep 巧妙融合二者优势：使用类似 AST 的简洁匹配语法，同时保留 CST 的格式信息
- 示例：`a = b + c` 和 `a=b+c;` 具有相同的 AST，但不同的 CST

## Tree-sitter 基础

### 解析引擎
- ast-grep 基于 Tree-sitter 解析器生成 CST
- Tree-sitter 支持多种语言，提供增量解析和错误恢复
- 解析结果包含 named 节点和 unnamed 节点

### Named vs Unnamed Nodes
- **Named Nodes**：具有语义意义的节点，如函数调用、变量声明
- **Unnamed Nodes**：语法结构节点，如括号、逗号、运算符
...
### 与传统工具的对比
- **文本搜索（grep/sed）**：无法理解代码结构，容易出现误匹配
- **AST 工具（ESLint/jscodeshift）**：学习成本高，需要深入了解编译原理
- **ast-grep**：结合两者优势，使用类似原生代码的语法描述 AST 结构
...
- 强大的规则系统：支持原子规则、关系规则和组合规则

## 应用场景

### 代码重构
- 从 Options API 迁移到 Composition API（Vue SFC）
- 函数重命名和签名变更
- 代码风格统一
- 依赖替换和升级
