...
desc: 第一次使用时先把命令装上；独立安装器安装的 uv 支持自更新，其他方式需走各自包管理器升级。
---
...
- workspace 可用 `uv run --all-packages` 在所有成员环境下运行命令
...
### 项目管理
- 创建项目：`uv init demo`
- 创建脚本：`uv init --script demo.py --python 3.12`
- 加依赖：`uv add httpx`
- 给脚本加依赖：`uv add --script demo.py httpx`
- 锁定依赖：`uv lock`
- 同步环境：`uv sync --locked`
- 导出锁文件：`uv export -o requirements.txt`
...
- 指定解释器：`uv run --python 3.12 python -V`
...
- 列出已安装：`uv python list --only-installed`

### 工具管理
- 列出已安装工具：`uv tool list`
- 升级工具：`uv tool upgrade ruff`
- 卸载工具：`uv tool uninstall ruff`

### CI / 缓存
- 按锁文件同步：`uv sync --locked --all-extras --dev`
- 清理缓存：`uv cache prune --ci`
...
- `uvx` 是 `uv tool run` 的别名，两者完全等价，用于临时执行未安装的工具
...
- 独立安装器安装的 `uv` 才支持 `uv self update`；pip / pipx / brew 等方式升级需走各自包管理器

## uv vs 全局 Python：边界与定位
---
emoji: 🌐
link: https://docs.astral.sh/uv/guides/install-python/
desc: uv 可以覆盖 Python 工具链的大部分能力，但它不是一个以"全局 python 命令"为核心的版本管理器。
---
### uv 能替代什么

| 传统工具 | uv 替代方案 | 说明 |
|---------|-------------|------|
| pip | `uv pip install` | 基本兼容，默认作用于虚拟环境 |
| venv | `uv venv` | 创建更快，与 uv 运行时无缝集成 |
| pipx | `uv tool install` / `uvx` | 长期工具用 install，一次性工具用 uvx |
| pyenv | `uv python install` | 按需安装，不提供"全局切换版本"的语义 |
...
- 是否能直接用 `python`，取决于 PATH 配置
- `--default` 能力仍可能是实验性特征，不应作为唯一依赖
