...
| `cdp_navigation_timeout` | CDP 导航超时（秒） | 新增于 0.12.6 |
...
browser-use setup
browser-use install
browser-use install-lite   # 轻量安装脚本
browser-use init --list
browser-use init --template default --output my_script.py
browser-use init --template advanced --output my_agent.py
browser-use init --template tools --output my_tools.py
```
...
- 进阶参数：`cloud_timeout`
- 版本：官方当前 release / PyPI 都是 `0.12.6`
...
| 参数 | 作用 | 备注 |
|---|---|---|
| `max_failures` | 允许失败次数 | 默认 3 |
| `step_timeout` | 单步超时（秒） | 默认 120 |
| `llm_timeout` | LLM 调用超时（秒） | 默认 90 |
| `use_vision` | 是否启用视觉 | `True/False/auto` |
| `use_thinking` | 启用内部思考 | 默认 True |
| `flash_mode` | 快速模式（跳过思考/评估） | 默认 False |
| `enable_planning` | 是否启用规划 | 某些模型/云场景会关 |
| `max_actions_per_step` | 每步最大动作数 | 默认 4，太多会跑偏 |
| `max_history_items` | LLM 记忆中的最大步数 | 默认全部保留 |
...
| `generate_gif` | 生成操作 GIF | `True` 或路径 |
| `calculate_cost` | 计算并跟踪 API 成本 | 默认 False |
| `fallback_llm` | 主 LLM 失败时的备份 | 429/401/402/500-504 错误时触发 |
