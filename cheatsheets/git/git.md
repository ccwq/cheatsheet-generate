...
## Recipe 7：子模块的引入、初始化与更新
---
emoji: 🧩
link: https://git-scm.com/docs/git-submodule
desc: 子模块适合把外部项目当作“固定在某个提交上的依赖”来管理。主仓库只保存指针，不直接吞进整个子项目历史。
---
- `git submodule add <url> <path>`：把外部项目接进当前仓库
- `git submodule status`：查看子模块当前指针状态
- `git clone --recursive <main-repo-url>`：克隆主仓库时顺便拉全子模块
- `git submodule update --init --recursive`：克隆后补初始化与递归拉取
- `git submodule update --remote <path/to/submodule>`：只更新某一个子模块
- `git submodule update --remote`：更新所有子模块
- `git pull --recurse-submodules`：主仓库和子模块一起拉
- `git rm <path>`：移除子模块，随后还要清理 `.gitmodules`

```bash
# 添加子模块后，记得把主仓库里的指针提交出去
...
### 常见坑
- 主仓库只记录子模块的提交指针，不会自动把子项目最新代码吸进来
- 更新完子模块后，必须回到主仓库 `git add` 并 `git commit`，团队成员才能拿到新版本
- 如果克隆后目录是空的，优先检查是不是忘了 `--recursive` 或 `update --init --recursive`
...
emoji: 🛠️
link: https://git-scm.com/docs/git-tag
desc: 这组属于“发布与维护层”。包括版本标签、补丁搬运、责任定位、忽略规则和清理命令。
---
- `git tag` : 列所有标签
