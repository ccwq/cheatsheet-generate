...
description: 更新本仓库已有 cheatsheet 的专用技能。用于用户提到更新、升级、同步新版、补 changelog、release notes、跨版本变化、把某个 topic 更新到最新版本时；必须优先使用本技能，而不是直接用 cheatsheet-maker。该技能会先调用 cheatsheet-review-skill 做现状评估，再结合 cheatsheet-maker 的正文与文件规范直接提升内容：处理过时命令/API/配置，补充新特性、迁移影响与高频用法，并在符合条件时维护正文末尾的版本变更模块与 changelog-history.md。
---

# cheatsheet-update

为已有 `cheatsheets/<topic>/` 执行版本更新、跨版本差异整理、正文内容提升、过时内容修正、元数据同步与版本变更归档。
...
- 根据 review 建议提升正文质量，不只维护 changelog
- 基于官方来源更新 `<topic>.md`、`meta.yml`、`refmap.md`
- 删除或改写过时命令、参数、API、配置项、工作流与兼容性说明
- 补充新版本的重要特性、新增命令/API/配置、高频用法和迁移注意事项
...
本技能是执行型更新技能，不是只输出评估报告，也不是只追加版本变更模块。review 结论和官方新版本资料必须转化为实际内容改动。

### 必须处理的正文改动

- 过时内容：移除或改写旧版本命令、废弃 API、失效配置、旧行为默认值
- 新特性：补充新版本中对高频使用有价值的功能、命令、参数、API、配置入口
- 迁移影响：说明破坏性变化、重命名、默认值变化、兼容性要求和替代方案
- 内容匮乏：按 review 建议补齐缺失的最小工作流、高频 recipes、参数对照或常见坑
- 结构失配：必要时按 cheatsheet-maker 的风格规则重排章节，但不要为了更新而重写全部正文
- 来源补强：正文新增或修正的关键结论必须能在 `refmap.md` 找到来源

### 改动优先级

1. 先修正会误导用户的过时内容和事实错误
2. 再补新版本中影响使用路径的新特性和迁移点
3. 再补 review 指出的内容匮乏项
4. 最后维护 `meta.yml`、frontmatter、`refmap.md` 和 `## 🧾 版本变更`

如果 review 建议与 release notes 信号冲突，以官方最新文档和最新稳定版本行为为准，并在结果中说明取舍。
...
6. 决定更新范围：
   - 正文中过时的命令、API、配置、工作流
   - review 指出的内容匮乏、结构失配、事实错误、来源缺失
   - 新版本带来的新特性、新参数、新配置、新工作流
   - 破坏性变化、迁移步骤、废弃项和替代方案
...
- 已先评审，再更新，无需等用户确认
- 已把 review 建议落实为正文改动，而不是停留在评估报告
- 已处理过时内容、事实错误和版本行为变化
- 已补充新版本的重要特性、迁移影响或新增高频用法
- 已核验版本来源，不靠记忆判断最新版本
