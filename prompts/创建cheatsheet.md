...
1. 基本原则：
   ...
   - 必须为该 cheatsheet 生成/更新 `meta.yml`，并包含以下字段：
     - `desc`: 一句话简介
     - `tags`: 标签列表
     - `version`: 整理时参考/采集的信息版本（不是“最新版本”）；不确定可写 "unknown"
     - `github`: 上游 GitHub 仓库，格式固定为 `owner/repo`；不确定可写 "unknown"
     - `date`: cheatsheet 创建日期，格式固定为 `YYYY-MM-DD`（新建时使用创建当天日期）
   - 详情页顶部必须展示 meta 信息：版本 / 更新日志 / GitHub（从 `meta.yml` 提取，不要硬编码）
